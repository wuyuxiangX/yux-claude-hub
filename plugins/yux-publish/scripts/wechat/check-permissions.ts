...
async function checkApiCredentials(): Promise<void> {
  ...
  const projectEnv = path.join(cwd, '.yux-publish', '.env');
  const userEnv = path.join(os.homedir(), '.yux-publish', '.env');
...
async function main(): Promise<void> {
  console.log('=== yux-publish-wechat: Permission & Environment Check ===\n');
