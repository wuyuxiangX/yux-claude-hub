...
export function highlightAndFormatCode(text: string, language: string, hljs: any, showLineNumber: boolean): string {
  ...
  if (showLineNumber) {
    ...
    const codeLinesHtml = `<div style="white-space:pre-wrap;word-wrap:break-word;line-height:1.75">${codeInnerHtml}</div>`
    ...
    highlighted = `<section style="padding:0"><div style="white-space:pre;line-height:1.6">${formatted}</div></section>`
  }
