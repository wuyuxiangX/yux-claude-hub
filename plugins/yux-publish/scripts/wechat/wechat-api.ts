...
function loadConfig(): WechatConfig {
  const cwdEnvPath = path.join(process.cwd(), ".yux-publish", ".env");
  const homeEnvPath = path.join(os.homedir(), ".yux-publish", ".env");
  ...
  if (!appId || !appSecret) {
    throw new Error(
      ...
      "Set via environment variables or in .yux-publish/.env file."
    );
  }
...
function loadExtendConfig(): Record<string, string> {
  const paths = [
    path.join(process.cwd(), ".yux-publish", "yux-publish-wechat", "EXTEND.md"),
    path.join(os.homedir(), ".yux-publish", "yux-publish-wechat", "EXTEND.md"),
  ];
...
Options:
  ...
  --theme <name>      Theme name for markdown (default, grace, simple, yudesk). Default: yudesk
...
function extractProp(tag: string, propName: string): string {
  // Try quoted values first: prop="value" or prop='value'
  const quotedMatch = tag.match(new RegExp(`${propName}\\s*=\\s*["']([^"']*)["']`));
  if (quotedMatch) return quotedMatch[1] || '';
  // Try JSX expression: prop={...}
...
function buildBrandingHeader(): string {
  return `<p style="text-align: center; margin: 0 0 1.5em;"><img src="http://wyx-shanghai.oss-cn-shanghai.aliyuncs.com/2026-02/ceb26eb177851233ad3652e4268778b13150b316.png" style="width: 100%; max-width: 400px;" /></p>`;
}

function buildBrandingFooter(): string {
  return `<p style="text-align: center; margin: 2em 0 0;"><img src="http://wyx-shanghai.oss-cn-shanghai.aliyuncs.com/2026-02/b0d78eae165378dc4e6c1a08ecfbb3812377b63f.png" style="width: 100%; max-width: 400px;" /></p>`;
}
...
function preprocessMdx(content: string): string {
  ...
  // 7. Interactive diagram components → static images
  const diagramImageMap: Record<string, string> = {
    // Add project-specific component-to-image mappings here
    // Example: 'MyDiagram': 'https://example.com/my-diagram.png',
  };
  for (const [component, imageUrl] of Object.entries(diagramImageMap)) {
    result = result.replace(new RegExp(`<${component}\\s*/>`, 'g'), `![${component}](${imageUrl})`);
  }

  // 8. Remove JSX comments: {/* ... */}
  result = result.replace(/\{\/\*[\s\S]*?\*\/\}/g, '');

  // 9. Remove remaining self-closing JSX tags (capitalized component names)
  result = result.replace(/<[A-Z][a-zA-Z]*\s[\s\S]*?\/\s*>/g, '');

  // 10. Convert internal relative links to plain text
  result = result.replace(/\[([^\]]+)\]\(\/(blog|docs)\/[^)]*\)/g, '$1');

  // 11. Remove import statements
  result = result.replace(/^import\s+.+$/gm, '');

  // 12. Remove export statements
...
async function main(): Promise<void> {
  ...
  if (args.isHtml) {
    ...
    // Remove h1 title from body — WeChat displays title separately via API
    htmlContent = htmlContent.replace(/<h1[^>]*>[\s\S]*?<\/h1>\s*/, '');
    htmlContent = buildBrandingHeader() + htmlContent;
    ...
    // Preprocess MDX components to standard Markdown
    const processedBody = preprocessMdx(body);

    // Write to temp file for rendering
    // Preserve original frontmatter text to avoid YAML serialization issues
    // (e.g. arrays like keywords: ["a", "b"] would break with naive quoting)
    const tmpMdPath = path.join(os.tmpdir(), `wechat-md-${Date.now()}.md`);
    const fmMatch = content.match(/^(---\r?\n[\s\S]*?\r?\n---\r?\n)/);
    const rawFrontmatter = fmMatch ? fmMatch[1] : '';
    fs.writeFileSync(tmpMdPath, rawFrontmatter + processedBody, "utf-8");

    console.error(`[wechat-api] Theme: ${args.theme}`);
    htmlPath = renderMarkdownToHtml(tmpMdPath, args.theme);
    ...
    try { fs.unlinkSync(tmpMdPath); } catch {}
  }

  // Append branding footer image
  htmlContent += buildBrandingFooter();

  // Decode HTML entities that WeChat doesn't render in code blocks
  // Note: do NOT replace &quot; — it's used in HTML attributes and would break the markup
  htmlContent = htmlContent
    .replace(/&#x27;/g, "'")
    .replace(/&#39;/g, "'")
    .replace(/&#96;/g, "`");

  // WeChat post-processing: fix rendering issues and strip unsupported content
  // 1. Strip mac-sign decorations (display:none macOS traffic light SVGs)
  htmlContent = htmlContent.replace(/<span class="mac-sign"[\s\S]*?<\/span>/gi, '');
  // 2. Strip fragment anchor links — WeChat API rejects href="#..." (error 45166)
  htmlContent = htmlContent.replace(/<a\s+href="#[^"]*"[^>]*>([\s\S]*?)<\/a>/gi, '$1');
  // 3. Strip redundant font-family from wrapper sections (WeChat sets its own)
  htmlContent = htmlContent.replace(/font-family:\s*-apple-system-font[^;]*;\s*/g, '');
  // 4. Clean up empty style attributes
  htmlContent = htmlContent.replace(/ style="\s*"/g, '');
  // 5. Ensure table elements have inline styles for WeChat (class selectors don't work)
  htmlContent = htmlContent.replace(
    /<table[^>]*class="preview-table"[^>]*>/gi,
    '<table style="border-collapse: collapse; margin: 1em auto; width: 100%; border: 1px solid #e8e8e8;">'
  );
  // 6. Strip empty <caption> tags (WeChat may render these as empty boxes)
  htmlContent = htmlContent.replace(/<caption[^>]*>\s*<\/caption>/gi, '');
  // 7. Replace all <section> with <div> — WeChat renders <section> with visible dashed borders,
  //    but <div> renders cleanly while preserving all inline styles (overflow, flex, etc.)
  htmlContent = htmlContent.replace(/<section\b/gi, '<div');
  htmlContent = htmlContent.replace(/<\/section>/gi, '</div>');

  // Fallback to EXTEND.md default_author
  if (!author) {
    const extendConfig = loadExtendConfig();
    if (extendConfig.default_author) {
